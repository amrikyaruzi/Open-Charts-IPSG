...
gs4_auth(email = "amrikyaruzi@gmail.com")
...
sheets_to_read <- sheets[c(5:7)] #reads sheets 5 to 7
...
# ### Converting all column values to "Y" and "N" for DOCUMENTED, LEGIBILITY, COMPLETENESS AND MET columns (use below instead)
# 
...
wrangle.it <- function(dataframe, column){
  ...
  intermediate <- intermediate %>% arrange(desc(Percentage)) %>% mutate_if(is.numeric,
                                                                           round_half_up, 1)
...
# Preparing the data
  ...
  read_pptx(here("./R Code/Template.pptx")) %>%
    ph_with(value = block_list(fpar(ftext(glue("{department} - OPA Documentation Compliance for {thequarter} {theyear}"),
...
###Exporting graphs per Consultant
    ...
    #Saving the pptx
    read_pptx(here("./R Code/Template.pptx")) %>%
      ph_with(value = block_list(fpar(ftext(glue("{consultant} - OPA Documentation Compliance for {thequarter} {theyear}"),
                                            prop = fp_text(font.size = 18, font.family = "Helvetica")),
                                      fp_p = fp_par(text.align = "center"))),
              location = ph_location_type(type = "subTitle")) %>% 
...
nurses <- data %>% filter(TYPE == "N")
  ...
  #Saving the pptx
  read_pptx(here("./R Code/Template.pptx")) %>%
    ph_with(value = block_list(fpar(ftext(glue("FMC Nurses - OPA Documentation Compliance for {thequarter} {theyear}"),
                                          prop = fp_text(font.size = 18, font.family = "Helvetica")),
                                    fp_p = fp_par(text.align = "center"))),
            location = ph_location_type(type = "subTitle")) %>% 
...
### Institutional
{
  ...
  #Saving the pptx
  read_pptx(here("./R Code/Template.pptx")) %>%
    ph_with(value = block_list(fpar(ftext(glue("Institutional OPA Documentation Compliance for {thequarter} {theyear}"),
                                          prop = fp_text(font.size = 18, font.family = "Helvetica")),
                                    fp_p = fp_par(text.align = "center"))),
            location = ph_location_type(type = "subTitle")) %>% 
